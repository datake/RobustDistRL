...
def run_steps(agent, args):
    ...
    while True:
        ...
        if config.log_interval and (agent.total_steps % config.log_interval == 0):
            agent.logger.info('steps %d, %.2f steps/s' % (agent.total_steps, config.log_interval / ( - t0))) # only compute speed
            t0 = time.time()
        if config.max_steps and agent.total_steps >= config.max_steps: # if count total_steps > max_steps, break!
